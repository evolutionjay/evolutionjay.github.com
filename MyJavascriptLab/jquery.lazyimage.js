(function($, window) {
    ...
    function isInViewport(el) {
        ...
        return !(viewport.right < bounds.left || viewport.left > bounds.right || viewport.bottom < bounds.top || viewport.top > bounds.bottom) && el.is(":visible");
    }
    ...
    $.LazyImage.prototype = {
        init:function() {
            var self = this, fadeIn = this.fadeIn;
            self.$fakeImg = $("<img />").on("load", function() {
                self.$img.removeAttr("style").css("opacity", 0).attr("src", $(this).attr("src"));
                if (fadeIn) {
                    self.$img.animate({
                        opacity:1
                    });
